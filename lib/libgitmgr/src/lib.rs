//! **libgitmgr** provides the ability to find a minimal set of user-relevant information for git
...
mod tests {
    ...
    /// This scenario test for `gitmgr` covers an end-to-end usage scenario. It uses the
    ...
    fn scenario() -> anyhow::Result<()> {
        ...
        if let Err(e) = repository.remote("origin", "https://github.com/trinhminhtriet/gitmgr") {
        ...
        if let Err(e) = repository.remote("fork", "https://github.com/trinhminhtriet/gitmgr") {
            if e.code() != ErrorCode::Exists {
                return Err(e.into());
            }
        }
        ...
        let mut nested_expected_views_raw = vec![
            RepositoryView::finalize(
                ...
                Some("HEAD".to_string()),
                Status::Clean,
                Some("https://github.com/trinhminhtriet/gitmgr".to_string()),
                None,
                Vec::with_capacity(0),
            )?,
            RepositoryView::finalize(
