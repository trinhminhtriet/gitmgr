//! This is the loose bench for gitmgr. Why use a loose bench over a precise one? Most of gitmgr
...
//! gitmgr is via CLI and not a library. Thus, this loose bench executes gitmgr as a CLI similarly
...
// We want gitmgr to perform consistently as well as quickly.
...
impl TaskRunner for RunLooseBench {
    fn run(harness: &mut TaskHarness) -> TaskResult<()> {
        ...
        let release = harness.root.join("target").join("release").join("gitmgr");
        let home = dirs::home_dir().ok_or(TaskError::HomeDirectoryNotFound)?;
        let installed = home.join(".cargo").join("bin").join("gitmgr");
        let target = harness
            .root
            .parent()
...
impl TaskHarness {
    fn loose_bench(
        ...
        let first_duration = self.execute_gitmgr(first, target)?;
        let second_duration = self.execute_gitmgr(second, target)?;
    ...
    fn execute_gitmgr(&self, binary: &Path, target: &Path) -> TaskResult<Duration> {
